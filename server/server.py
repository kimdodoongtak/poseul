...
async def register(data: RegisterRequest, request: Request):
    ...
    try:
        with engine.connect() as conn:
            ...
            # 비밀번호 해싱
            password_bytes_len = len(data.password.encode('utf-8')) if data.password else 0
            logger.info(f"🔐 회원가입 - 아이디: {data.id}, 비밀번호 길이: {len(data.password)}자, 바이트: {password_bytes_len}바이트")
